...
def get_kfp_client(
    ...
    host_value = _ensure_pipeline_host(
        host or os.getenv("KFP_HOST", "http://localhost:8080/pipeline")
    )
    namespace_value = namespace or os.getenv("KFP_NAMESPACE", "user-example-com")
    cookie_value = cookie or os.getenv("KFP_COOKIE")
    token_value = token or os.getenv("KFP_TOKEN")
    username_value = username or os.getenv("DEX_USERNAME", "user@example.com")
    password_value = password or os.getenv("DEX_PASSWORD", "12341234")
