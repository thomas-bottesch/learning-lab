...
def get_gpu_temp(last_ema: Optional[float] = None) -> float:
    ...
    try:
        ...
        logging.error(f"Failed to read GPU temperature: {e}. Using fallback temp of 90°C")
        temp_c = 90
...
def get_cpu_temp(cpu_temp_path: str, last_ema: Optional[float] = None) -> float:
    try:
        ...
        logging.error(f"Failed to read CPU temperature from {cpu_temp_path}: {e}. Using fallback temp of 90°C")
        return 90000

def determine_fan_speed_percent(cpu_temp_path: str, last_percent: Optional[float] = None, last_gpu_ema: Optional[float] = None, last_cpu_ema: Optional[float] = None) -> Tuple[float, float, float]:
    try:
        CPU_MIN = 50 * 1000
        CPU_MAX = 70 * 1000
        GPU_MIN = 55 * 1000
        ...
        gpu_percent = (min(max(gpu_temp, GPU_MIN), GPU_MAX) - GPU_MIN) / (GPU_MAX - GPU_MIN)
        cpu_percent = (min(max(cpu_temp, CPU_MIN), CPU_MAX) - CPU_MIN) / (CPU_MAX - CPU_MIN)
        ...
        if last_percent is not None:
            ...
            if percent_change < HYSTERESIS_PERCENT:
                ...
                logging.debug(f"Fan speed calculation: GPU={gpu_percent*100:.1f}%, CPU={cpu_percent*100:.1f}%, Calculated={calculated_percent*100:.1f}% (suppressed by hysteresis, keeping {last_percent*100:.1f}%)")
            else:
                logging.debug(f"Fan speed calculation: GPU={gpu_percent*100:.1f}%, CPU={cpu_percent*100:.1f}%, Using={max_percent*100:.1f}% (changed from {last_percent*100:.1f}%)")
        else:
            logging.debug(f"Fan speed calculation: GPU={gpu_percent*100:.1f}%, CPU={cpu_percent*100:.1f}%, Using={max_percent*100:.1f}%")
...
def cleanup_and_exit(main_path: Path, signum: Optional[int] = None, frame: Optional[FrameType] = None) -> None:
    ...
    for pwm_num in PWM_CHANNELS:
        ...
        try:
            ...
            logging.error(f"Failed to restore automatic fan control for pwm{pwm_num}: {e}")
...
def main() -> None:
    ...
    # Check if enabled if not exit with error
    for pwm_num in PWM_CHANNELS:
        pwm_enable = main_path / f"pwm{pwm_num}_enable"
        try:
            with open(pwm_enable, "r") as f:
                current_value = f.read().strip()
            if current_value != "1":
                logging.critical(f"Failed to enable manual fan control for pwm{pwm_num}")
                cleanup_and_exit(main_path)
                sys.exit(1)
        except Exception as e:
    ...
    logging.info(f"Manual fan control enabled successfully for PWM channels: {PWM_CHANNELS}")
    ...
    while True:
        percent, gpu_ema, cpu_ema = determine_fan_speed_percent(str(cpu_temp_path), last_percent, last_gpu_ema, last_cpu_ema)
        ...
        for pwm_num in PWM_CHANNELS:
            ...
            try:
                ...
                logging.error(f"Failed to write to pwm{pwm_num}: {e}")
