...
def validate_output(original_prompt, content, response):
    llm_validator = OllamaLLM(model="phi3:mini-128k", temperature=0.0, num_predict=100)
    validation_prompt = f"""
    ...
    l_validation_response = validation_response.lower()
...
def extract_story_metadata(story_file_path: str) -> list[str]:
    ...
    # Initialize Ollama LLM with slightly higher temperature for creativity
    llm = OllamaLLM(model="phi3:mini-128k", temperature=0.7, num_predict=150)
...
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            json_obj = parser.parse(response)
